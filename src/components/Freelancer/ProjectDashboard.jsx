...
const ProjectDashboard = () => {
  ...
  const [savedProjects, setSavedProjects] = useState({});  const [sortOption, setSortOption] = useState('latest');
    ...
    <div className="min-h-screen bg-[#FFECFB] flex flex-wrap p-4">
      ...
      <div className="w-full md:w-3/4 p-4">
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          <h2 className="text-base font-semibold mb-4 text-[#333399]">{filteredProjects.length} Projects</h2>
          </div>
          
          {/* Alert Enable Button */}
          <div className="flex justify-end space-x-3">
          <div
            className={`flex items-center cursor-pointer ${alertEnabled ? 'text-[#9919B2]' : 'text'}`}
            onClick={() => setAlertEnabled(!alertEnabled)}
          >
            {alertEnabled ? <HiBell size={20} /> : <HiOutlineBell size={20} />}
            <span className="ml-2 text-base font-semibold">Notification alert</span>
          </div>
        ...
        <div>
          <ul className="space-y-4">
            {filteredProjects.map((project) => (
              <li key={project.id} className="p-4 bg-white shadow rounded-lg cursor-pointer hover:bg-gray-100">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-lg font-semibold text-[#FF00CC]">{project.biddingAmount}</span>
                  </div>
                </div>
                <p className="text-sm mb-2">{project.description}</p>
                <p className="text-sm text-[#333399] font-semibold mb-2">* {project.skills.join(' * ')}</p>
                <div className="flex justify-end items-center space-x-3 mt-4">
                  ...
                  >
                    {savedProjects[project.id] ? <FaBookmark size={20} /> : <FaRegBookmark size={20} />}
                  </div>
                </div>
              </li>
            ))}
      ...
      <div className="w-full md:w-1/4 p-4">
        ...
        <div className="mb-6 p-4 bg-white shadow rounded-lg">
          ...
          <p className="text-gray-600">Current working project description...</p>
        </div>
            ...
            <div className="mt-2 space-y-2">
              ...
              {projectType === 'hourRate' && (
                <div className="ml-6 mt-2">
                  <label className="block text-gray-700">Rate: {hourRateRange} USD/hour</label>
                  <input
                    type="range"
                    min="10"
                    max="200"
              ...
              {projectType === 'fixedRate' && (
                <div className="ml-6 mt-2">
                  <label className="block text-gray-700">Rate: {fixedRateRange} USD</label>
                  <input
                    type="range"
                    min="100"
                    max="10000"
